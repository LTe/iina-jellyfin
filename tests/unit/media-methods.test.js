...
function expectAuthed(fetchMock, index = 0) {
  ...
  expect(options.headers.Authorization).toMatch(
    /^MediaBrowser Client="IINA Jellyfin Plugin", .*Token="tok"$/
  );
  ...
  describe('rendering media items', () => {
    ...
    it('picks thumbnails by item type and available images', () => {
      ...
      expect(thumb({ ...EPISODE, ImageTags: { Primary: 'p' } })).toBe(
        `${BASE}/Items/ep-1/Images/Primary?maxWidth=160&quality=90&ApiKey=tok`
      );
      expect(thumb(EPISODE)).toBe(
        `${BASE}/Items/series-1/Images/Thumb?maxWidth=160&quality=90&ApiKey=tok`
      );
      expect(thumb({ ...EPISODE, SeriesId: undefined, ImageTags: { Thumb: 't' } })).toBe(
        `${BASE}/Items/ep-1/Images/Thumb?maxWidth=160&quality=90&ApiKey=tok`
      );
  ...
  describe('playback', () => {
    it('builds stream urls', () => {
      expect(sidebar.buildStreamUrl(MOVIE)).toBe(
        `${BASE}/Videos/movie-1/stream?static=true&ApiKey=tok`
      );
      expect(sidebar.buildStreamUrl(SONG)).toBe(
        `${BASE}/Audio/song-1/stream?static=true&ApiKey=tok`
      );
