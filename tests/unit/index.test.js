...
describe('plugin main entry', () => {
  ...
  describe('file loading', () => {
    ...
    describe('connected account reporting', () => {
      ...
      it('reports to the connected account on the same host', async () => {
        ...
        expect(fake.iina.http.post).toHaveBeenCalledWith(
          'https://JF.local:8096/Sessions/Playing?ApiKey=account-token',
          expect.anything()
        );
        expect(logged(fake)).toContain(
      ...
      it('treats a server behind a sub path as the same host', async () => {
        ...
        expect(fake.iina.http.post).toHaveBeenCalledWith(
          'http://jf.local:8096/jellyfin/Sessions/Playing?ApiKey=account-token',
          expect.anything()
        );
      });
  ...
  describe('playback lifecycle', () => {
    ...
    it('reports stop when the file ends', async () => {
      ...
      await flushPromises();
      expect(fake.iina.http.post).toHaveBeenCalledWith(
        `${SERVER}/Sessions/Playing/Stopped?ApiKey=${KEY}`,
        expect.anything()
      );
    });
    ...
    it('reports pause changes', async () => {
      ...
      await flushPromises();
      expect(fake.iina.http.post).toHaveBeenCalledWith(
        `${SERVER}/Sessions/Playing/Progress?ApiKey=${KEY}`,
